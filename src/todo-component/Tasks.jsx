...
export default function Tasks() {
  ...
  const saveTask = async (taskName) => {
    if (taskName.length > 0) {
      try {
        await axios.post(
          'https://functions-cust-fun-sandy.harperdbcloud.com/api/saveTask',
  ...
  const deleteTask = async (task) => {
    try {
      await axios.delete(
        `https://functions-cust-fun-sandy.harperdbcloud.com/api/deleteTask/${task.id}`
      );
      getTasks();
    } catch (ex) {
        showToast();
  ...
  const getTasks = async () => {
    ...
    const res = await axios.get(
      'https://functions-cust-fun-sandy.harperdbcloud.com/api/tasks'
    );
    console.log(res);
